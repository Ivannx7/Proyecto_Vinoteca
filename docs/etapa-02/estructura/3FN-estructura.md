...
## 	Venta:
° 1FN: todos los valores de sus columnas (fecha, total, estado, etc.) son atómicos. No existen listas de productos ni grupos repetidos dentro de la cabecera de la venta
...
° 3FN: Todos sus atributos no clave (fecha, total, estado, id_cliente, id_usuario_vendedor, id_usuario_gerente_autoriza) dependen única y directamente del identificador de venta (id_venta). Ningún atributo no clave depende de otro que no sea la clave primaria.

## Usuario: 
°1FN: todos los atributos (nombre, email, contraseña, etc.) almacenan valores atómicos e indivisibles.
...
## Rol:
°1FN: sus atributos (nombre, descripción, etc.) almacenan valores atómicos e indivisibles.
°2FN: su clave primaria es simple (id_rol), sin riesgo de dependencias parciales.
°3FN: atributos como (nombre_rol, descripción y estado) describen única y directamente al rol identificado por (id_rol).

## Cliente:
°1FN: cada columna (nombre, apellido, dni, email, etc.) almacena un valor único simple.
°2FN: tiene una clave primaria simple (id_cliente).
°3FN: todos los datos personales corresponden de forma exclusiva al cliente identificado por (id_cliente).

## Categoría:
°1FN: Los atributos de la categoría son atómicos (nombre_categoria, descripción).
°2FN: posee clave primaria atómica (id_categoria), lo que elimina la posibilidad de tener dependencias parciales.
°3FN: los atributos (nombre_categoria y descripción) dependen única y directamente del (id_categoria).

## Bodega:
°1FN: contiene valores atómicos como (descripción, nombre, etc).
°2FN: posee clave primaria simple (id_bodega).
...
## País:
°1FN: Los atributos de la categoría son atómicos.
°2FN: posee clave primaria simple (id_pais).
°3FN: El atributo (nombre) depende única y directamente del (id_pais).

## Región:
°1FN: Los atributos de la categoría son atómicos (región, nombre).
°2FN: posee clave primaria simple (id_region).
...
## Cepa:
°1FN: Los atributos de la categoría son atómicos (cepa, nombre).
°2FN: posee clave primaria simple (id_cepa).
°3FN: El atributo (nombre) de la variedad de uva depende única y directamente de (id_cepa).

## VinoCepa:
°1FN: Los valores en el porcentaje y en las claves son atómicos.
°2FN: posee clave primaria compuesta (id_vino, id_cepa). El atributo no clave (porcentaje) depende de la combinación completa del vino y la cepa (no se puede saber el porcentaje indicando solo el vino o solo la cepa)
...
## MetodoPago:
°1FN: Los atributos de la categoría son atómicos (método_pago).
°2FN: posee clave primaria simple (id_metodo_pago).
°3FN: El atributo (método_pago) depende única y directamente del identificador del  método (id_metodo_pago).

## PagoVenta:
°1FN: Los atributos de la categoría son atómicos (pago, venta, etc).
°2FN: posee clave primaria simple (id_pago).
...
## HistorialPrecio:
°1FN: todos sus registros son atómicos.
°2FN: posee clave primaria simple (id_historial).
...
## DetalleVenta:
°1FN: todos sus atributos son atómicos (cantidad, precio_unitario_historico, etc.).
°2FN: posee clave primaria simple (id_detalle_venta).
...
Solución: el campo (subtotal) debe eliminarse de la tabla y calcularse mediante una consulta cuando se requiera.

## Vinos:
°1FN: todos sus atributos son atómicos (nombre, cosecha, etc.).
°2FN: posee clave primaria simple (sku).
...
Solución: corregir el nombre de la clave foránea apuntando a Categoria(id_categoria) y agregar el campo id_bodega (FK->Bodega) en la tabla vinos.
